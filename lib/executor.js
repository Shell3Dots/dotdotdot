...
const { analyzeRisk, decideRunPolicy } = require('./safety');
...
function applyRunPolicy(command, explanation, warning, risk, config) {
...
async function interactiveMode(result, config, context) {
  ...
  const { warn, policy } = applyRunPolicy(command, explanation, warning, risk, config);
  const box = commandBox(command, explanation, warn);

  if (policy === 'block') {
    console.log();
    console.log(box);
    printError('High-risk command blocked. Re-run with --allow-dangerous to enable it.');
    cleanExit(1);
    return;
  }
  ...
  if (policy === 'insert') {
    ...
    process.stderr.write(`  ${dim('not a TTY — printed command, did not run. Pass --yes to execute.')}\n`);
  ...
  if (policy === 'execute') {
    ...
    if (code !== 0 && process.stdin.isTTY && !config?.yes) {
...
async function executeMode(result, config) {
  ...
  if (risk.level === 'high' && !config?.allowDangerous) {
    console.log(`  ${red(symbols.warning)} ${bold('Blocked.')} High-risk — pass --allow-dangerous to run.`);
    cleanExit(1);
  }

  // autoExec without --yes still refuses LLM-flagged warnings
  if (warning && !config?.yes && !config?.allowDangerous) {
    console.log(`  ${red(symbols.warning)} ${bold('Blocked.')} Use interactive mode or --yes.`);
    cleanExit(1);
  }
...
module.exports = { runCommand, copyToClipboard, interactiveMode, executeMode, handleFailedCommand, cleanExit, detectBestShell, stripShellWrapper, applyRunPolicy };
