...
/**
 * nonTty: 'cancel' (default — never auto-run), 'first', or a specific option key (e.g. 'q').
 */
function resolveNonTtySelection(options, nonTty = 'cancel') {
  if (nonTty === 'first') {
    const first = options.find(o => !o.disabled);
    return first ? first.key : null;
  }
  if (nonTty && nonTty !== 'cancel') {
    const match = options.find(o => o.key === nonTty && !o.disabled);
    return match ? match.key : null;
  }
  return null;
}

function selectMenu(options, opts = {}) {
  return new Promise((resolve) => {
    if (!process.stdin.isTTY) {
      return resolve(resolveNonTtySelection(options, opts.nonTty));
    }
...
module.exports = { selectMenu, confirm, textInput, resolveNonTtySelection };
